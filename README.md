...
Failed and stopped runs expose `Review & Resume` in the UI. The confirmation
shows the original immutable settings, then starts a distinct run with the same
generated workflow. Run history records the source run, while the workflow's
normal authoritative Git, GitHub, and PurpleMux inspection recovers existing
Base PR, plan, and child-PR state; the terminated Python process is not restored.
