...
class RunnerSnapshot:
    ...
    def _issue_driven_summary_json(self) -> dict[str, object] | None:
        ...
        repository_summaries = [
            self._repository_summary_json(repository) for repository in repositories
        ]
...
class PythonRunner:
    ...
    def configure_browser_origin(self, origin: str) -> None:
        """Set the authoritative browser-navigation origin for this Runner."""
        if not origin.startswith("http://") or "\0" in origin:
            raise ValueError("browser origin must be a local HTTP URL")
        with self._lock:
            if any(run.state == "running" for run in self._runs.values()):
                raise RuntimeError("browser origin cannot change while a run is active")
            self._browser_origin = origin.rstrip("/")

    def configure_event_endpoint(self, base_url: str) -> None:
        """Enable managed Workflow callbacks for an attached HTTP server."""
    ...
    def _accept_parsed_event(
        ...
        if event_type == "finding":
            ...
            if run.issue_driven_repositories:
                run.issue_driven_repositories[-1].integration_pr = run.integration_pr
            ...
            run.issue_driven_repositories.append(
                _IssueDrivenRepositoryRecord(run.issue_driven_context)
            )
        elif event_type == "issue_result":
            issue_result = cast(IssueResult, event)
            run.issue_results[issue_result.issue] = issue_result
            if run.issue_driven_repositories:
                run.issue_driven_repositories[-1].issue_results[issue_result.issue] = (
                    issue_result
                )
            ...
            if run.issue_driven_repositories:
                run.issue_driven_repositories[-1].issue_navigations[
                    issue_navigation.issue
                ] = issue_navigation
            ...
            if run.issue_driven_repositories:
                run.issue_driven_repositories[-1].planner_skips[accepted.issue] = (
                    accepted
                )
    ...
    def _notify_terminal(
        ...
        click_url = (
            f"{self._browser_origin}/?run={self._run_identity(run_id)}"
            if self._browser_origin is not None
            else None
        )
        try:
